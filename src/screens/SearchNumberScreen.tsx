...
import { searchNumber, searchNumberPdf, shareFile } from '../services/api';
...
export default function SearchNumberScreen() {
  ...
  async function handleSearch() {
    ...
    try {
      ...
      const count = 'groupedResults' in result
        ? result.groupedResults.reduce((sum, g) => sum + g.recordCount, 0)
        : result.totalCount;
      updateRequest(id, { status: 'success', data: result, recordCount: count });
    } catch (err: any) {
  ...
  function handleViewResults(req: SearchRequest) {
    if (req.data) {
      ...
      navigation.navigate('Results', { data: req.data, type: 'number' });
    }
  }
  ...
  async function fetchPdfUri(req: SearchRequest): Promise<string | null> {
    ...
    try {
      const uri = await searchNumberPdf({
  ...
  async function handleViewPdf(req: SearchRequest) {
    ...
    if (uri) {
      navigation.navigate('PdfViewer', { fileUri: uri, title: `CDR - ${req.phoneNumber}` });
    }
  }
  ...
  async function handleRetry(req: SearchRequest) {
    ...
    try {
      ...
      const count = 'groupedResults' in result
        ? result.groupedResults.reduce((sum, g) => sum + g.recordCount, 0)
        : result.totalCount;
      updateRequest(req.id, { status: 'success', data: result, recordCount: count });
    } catch (err: any) {
    ...
    >
      ...
      >
        ...
        <View style={styles.pageHeader}>
          <Text style={styles.pageTitle}>Recherche par Numéro</Text>
          <Text style={styles.pageSubtitle}>Analysez les CDR d'un numéro de téléphone</Text>
        </View>
        ...
        <View style={styles.formCard}>
          <View style={styles.fieldGroup}>
            ...
            <View style={[styles.inputWrapper, focusedField === 'phone' && styles.inputFocused]}>
              <Ionicons name="call-outline" size={18} color={focusedField === 'phone' ? '#667EEA' : 'rgba(255,255,255,0.3)'} />
              <TextInput
                style={styles.input}
        ...
        {requests.length > 0 && (
          <View style={styles.queueCard}>
            ...
            <View style={styles.tableHeader}>
              <Text style={[styles.thCell, { flex: 1.2 }]}>Numéro</Text>
              <Text style={[styles.thCell, { flex: 1.3 }]}>Période</Text>
              <Text style={[styles.thCell, { flex: 0.9, textAlign: 'center' }]}>Statut</Text>
              <Text style={[styles.thCell, { flex: 0.4, textAlign: 'center' }]}></Text>
            </View>
            ...
            {requests.map((req, idx) => (
              <View key={req.id}>
                <View style={[styles.tableRow, idx % 2 === 0 && styles.tableRowEven]}>
                  {/* Numéro */}
                  <Text style={[styles.tdCell, { flex: 1.2, fontWeight: '600' }]} numberOfLines={1}>
                    {req.phoneNumber}
                  </Text>

                  {/* Période */}
                  <Text style={[styles.tdCell, { flex: 1.3, fontSize: 11 }]} numberOfLines={1}>
                ...
                {req.actionsOpen && req.status === 'success' && (
                  <View style={styles.dropdown}>
                    <TouchableOpacity style={styles.dropdownItem} onPress={() => handleViewResults(req)} activeOpacity={0.7}>
                      <Ionicons name="list-outline" size={16} color="#667EEA" />
                    ...
                    <TouchableOpacity
                      ...
                      onPress={() => handleViewPdf(req)}
                      activeOpacity={0.7}
                      disabled={req.pdfLoading}
                    >
                      <Ionicons name="eye-outline" size={16} color="#34D399" />
                      <Text style={styles.dropdownText}>
                        {req.pdfLoading ? 'Chargement...' : 'Voir PDF'}
                      </Text>
                    </TouchableOpacity>
                    ...
                    <TouchableOpacity
                      style={styles.dropdownItem}
                      onPress={() => handleDownloadPdf(req)}
                      activeOpacity={0.7}
                      disabled={req.pdfLoading}
                    >
                      <Ionicons name="download-outline" size={16} color="#A78BFA" />
                      <Text style={styles.dropdownText}>
                        {req.pdfLoading ? 'Chargement...' : 'Télécharger PDF'}
                      </Text>
                    </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  circle1: {
    ...
    backgroundColor: 'rgba(102,126,234,0.08)', top: -80, right: -60,
  },
  scroll: {
  ...
  // Form
  formCard: {
    backgroundColor: 'rgba(255,255,255,0.06)', borderRadius: 22, padding: 20, marginBottom: 20,
  ...
  inputWrapper: {
    ...
    borderWidth: 1.5, borderColor: 'rgba(255,255,255,0.06)', gap: 10,
  },
  inputFocused: {
    borderColor: 'rgba(102,126,234,0.5)', backgroundColor: 'rgba(102,126,234,0.06)',
  },
  input: {
  ...
  searchButton: {
    ...
    height: 52, borderRadius: 16, gap: 10,
  },
  ...
  queueBadge: {
    backgroundColor: 'rgba(102,126,234,0.2)', paddingHorizontal: 10, paddingVertical: 3, borderRadius: 8,
  },
  queueBadgeText: { color: '#667EEA', fontSize: 12, fontWeight: '700' },
  ...
  thCell: {
    color: '#667EEA', fontSize: 10, fontWeight: '700',
